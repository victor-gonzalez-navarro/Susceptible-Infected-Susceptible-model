...
# ------------------------------------------------------------------------------- HYPERPARAMETERS SIS
possible_mu = [0.1, 0.5, 0.9]  # spontaneous recovery probability (infected -> susceptible)
...
# Betas with plots
betas_drawing = [0.18, 0.19, 0.20, 0.25, 0.40, 0.90]
    ...
    for file in f:
        ...
        for mu in possible_mu:
            ...
            for iteration in range(ro_over_time.shape[0]):
                plt.plot(ro_over_time[iteration,:], label='Beta = '+str(betas_drawing[iteration]))
            plt.legend()
